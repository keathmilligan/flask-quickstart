...
def api_fixture():
    ...
    with app.app_context():
        sample.db.init_db()
        with open("tests/sampledata.sql", "r", encoding="utf-8") as f:
            conn = db.engine.raw_connection()  # pylint: disable=no-member
...
def test_get(api_fixture):
    ...
    rv = client.get(
        "/api/contacts/1", headers={"Authorization": "Bearer " + access_token}
    )
    assert rv.status_code == 200
    data = json.loads(rv.data.decode("utf-8"))
...
def test_get_all(api_fixture):
    ...
    client, access_token = api_fixture
    rv = client.get(
        "/api/contacts", headers={"Authorization": "Bearer " + access_token}
    )
    assert rv.status_code == 200
    data = json.loads(rv.data.decode("utf-8"))
...
def test_post(api_fixture):
    ...
    client, access_token = api_fixture
    data = {"last_name": "Updated Last Name", "email": "updated.email@somewhere.com"}
    rv = client.post(
        "/api/contacts/10",
        data=json.dumps(data),
        content_type="application/json",
    ...
    rv = client.get(
        "/api/contacts/10", headers={"Authorization": "Bearer " + access_token}
    )
    assert rv.status_code == 200
    data = json.loads(rv.data.decode("utf-8"))
...
def test_delete(api_fixture):
    ...
    client, access_token = api_fixture
    rv = client.delete(
        "/api/contacts/100", headers={"Authorization": "Bearer " + access_token}
    )
    assert rv.status_code == 200
